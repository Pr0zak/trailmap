...
- **Load-on-pan** — the map refetches as you scroll. `MapScreen`'s camera-idle listener calls `vm.onCameraIdle(bounds, center)`; the ViewModel refetches once ~25% of the visible rectangle has no data behind it (`needsRefetch`, `EDGE_SLACK`), debounced 450 ms (120 ms when the area is already parsed in memory). Loads supersede each other (`loadJob.cancel()` + a `loadSeq` guard) and the HTTP call is cancelled with them. The **camera is decoupled from `ui.center`** — it's placed once, restored from `vm.lastCamera` after a tab switch, and only my-location / trail-system headers move it; following `center` would yank the map back and re-trigger the fetch it just answered. An **"Auto" chip** (persisted) turns pan-loading off; **"Search this area"** appears when the view is too wide to auto-cover (`canAutoCover`). The radius chips are a **floor**, not a ceiling: zooming out grows the fetch along `RADIUS_LADDER` up to 16 km, zooming in never shrinks it. MTB mode opts out of viewport-derived radii ("systems within 25 mi" is a search radius, not a description of the screen).
...
- `Locator.current()` calls `getCurrentLocation(PRIORITY_BALANCED_POWER_ACCURACY, null)` with a **null cancellation token and no timeout** — on a device with a cold GPS this can stall `bootstrap()` (and therefore the first trail load) for tens of seconds. Wrap it in `withTimeoutOrNull(~5 s)` with a real `CancellationTokenSource` and fall through to `lastLocation`.
